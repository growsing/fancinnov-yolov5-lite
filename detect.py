...
def status_loop(dl):
    """每隔1秒打印一次无人机状态（高度、电池、位置等），每条日志带时间戳"""
    while True:
        # 读取当前状态，若属性不存在则使用默认值
        ...
        # 打印状态，加上时间戳
        print(f"[{timestamp}] [Status] Alt: {alt:.2f}m | Batt: {batt_v:.1f}V {batt_i:.1f}A | "
              f"Pos: ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f}) | Yaw: {yaw:.2f}rad", '\n')
              
              
        
        # 电量预警：当电压低于6.7V且大于0（避免默认0.0误报）时输出明显警报
        if batt_v < 6.7 and batt_v > 0:
            # 使用ANSI转义码：红色加粗，醒目提示
            warning_msg = (f"\033[91m\033[1m!!! BATTERY LOW: {batt_v:.1f}V < 6.7V !!! PLEASE LAND !!!\033[0m")
            print(warning_msg)
            # 额外输出中文警告，更直观
            print("⚠⚠⚠️ 警告：电池电压过低，请立即降落！ ⚠⚠⚠️")
...
def detect(save_img=False):
    ...
    # 创建日志文件（与视频保存在同一目录），文件名包含开始时间
    log_file_path = save_dir / f'detection_log_{start_time_str}.txt'
    ...
    try:
        for path, img, im0s, vid_cap in dataset:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):
                    ...
                    for idx, (*xyxy, conf, cls) in enumerate(det):
                        ...
                        if idx == max_conf_idx:
                            ...
                            # 保存最佳目标信息（将类别名 person 映射为 drone，并增加醒目提示）
                            target_class = "drone"  # 统一显示为 drone
                            best_target_info = {
                                'class': target_class,
                                'conf': conf,
                                'depth': dz_m,
                ...
                view_img = 1
                if view_img:
                    # 固定窗口大小为 1280x720（可根据需要调整）
                    win_name = str(p)
                    cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)  # 允许调整大小
                    cv2.resizeWindow(win_name, 480, 320)        # 固定为 1280x720
            ...
            # 本帧所有检测处理完毕，如果有最佳目标，输出一行日志（带醒目提示）
            if best_target_info is not None:
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                # 添加醒目的提示符号
                log_line = (f"[{timestamp}] >>> DRONE DETECTED! <<< "
                            f"{best_target_info['class']} conf={best_target_info['conf']:.2f} "
                            f"depth={best_target_info['depth']:.2f}m "
                            f"dx={best_target_info['dx']:.3f}m "
                            f"dy={best_target_info['dy']:.3f}m "
                            f"yaw={best_target_info['yaw']:.3f}rad "
