#!/usr/bin/env python3
"""
detect_control.py — 单字母命令遥控 + YOLO 视觉跟踪 + MiDaS 实时深度估计（双窗口显示）
命令映射：
  a -> arm (解锁)      d -> disarm (上锁)
  t -> takeoff (起飞)  l -> land (降落)
  k -> track (启动视觉跟踪 + 深度估计，双窗口)
  h -> halt (悬停/停止跟踪)
  q -> 在跟踪过程中输入 → 停止跟踪，返回命令菜单

特性：
- 左侧窗口：YOLO 目标检测与跟踪（含控制量标绘）
- 右侧窗口：MiDaS_small 实时彩色深度图（Inferno 映射）
- 同时保存原始跟踪视频和深度视频
- 状态日志与深度日志独立保存
- 深度输出为 MiDaS 原始逆深度（视差），数值越大表示物体越近，越小表示越远（无量纲）
- Zigbee 通信：检测到目标时发送深度数据；持续接收其他无人机消息并显示
- 集群协同：根据归一化背景深度（背景深度/目标深度）决定前进/后退，保持环境中心
"""

import argparse
import time
from pathlib import Path
import threading
import select
import sys
import cv2
import numpy as np
import torch
import torch.backends.cudnn as cudnn
from numpy import random
from datetime import datetime

# ===== 新增：Zigbee 通信依赖 =====
import serial
import socket

# ===== 原有 YOLO 导入 =====
from models.experimental import attempt_load
from utils.datasets import LoadPicamera2, LoadImages
from utils.general import (check_img_size, check_requirements, check_imshow,
                           non_max_suppression, apply_classifier,
                           scale_coords, xyxy2xywh, strip_optimizer, set_logging,
                           increment_path)
from utils.plots import plot_one_box
from utils.torch_utils import select_device, load_classifier, time_synchronized

# ===== 树莓派摄像头与数据链 =====
from picamera2 import Picamera2
from datalink_serial import datalink

# ===== MiDaS 深度估计依赖 =====
try:
    import timm
except ImportError:
    raise ImportError("请先安装 timm: pip install timm")

# ================= Zigbee 配置 =================
ZIGBEE_PORT = '/dev/ttyUSB0'      # 请根据实际设备修改
ZIGBEE_BAUD = 9600
ZIGBEE_SEND_INTERVAL = 1.0        # 发送深度数据的间隔（秒）
# =============================================

midas_model = None
midas_transform = None
midas_device = None

# Zigbee 全局变量
zigbee_ser = None
zigbee_running = True
zigbee_last_send_time = 0
zigbee_lock = threading.Lock()

# 存储对方无人机的最新数据（目标深度和背景深度）
remote_target_depth = None
remote_bg_depth = None
remote_data_time = 0
remote_lock = threading.Lock()

# 本机标识
MY_ID = socket.gethostname()


def load_midas_once():
    """加载 MiDaS_small，缓存为全局变量"""
    global midas_model, midas_transform, midas_device
    if midas_model is not None:
        return
    midas_device = torch.device('cpu')
    print(f"正在加载 MiDaS_small 模型到 {midas_device} ...")
    midas_model = torch.hub.load("intel-isl/MiDaS", "MiDaS_small")
    midas_model.to(midas_device)
    midas_model.eval()

    midas_transforms = torch.hub.load("intel-isl/MiDaS", "transforms")
    midas_transform = midas_transforms.small_transform
    print("MiDaS_small 加载完成。")


def run_midas_full(rgb_image):
    """
    输入 RGB 图像 (H,W,3)，返回全分辨率深度图 (H,W) numpy 数组
    输出为 MiDaS 原始逆深度（视差），无量纲。
    数值含义：越大表示物体离相机越近，越小表示越远。
    """
    input_batch = midas_transform(rgb_image).to(midas_device)
    with torch.no_grad():
        prediction = midas_model(input_batch)
        prediction = torch.nn.functional.interpolate(
            prediction.unsqueeze(1),
            size=rgb_image.shape[:2],
            mode="bicubic",
            align_corners=False,
        ).squeeze()
    return prediction.cpu().numpy()


def depth_to_colormap(depth_map, min_depth=None, max_depth=None):
    """将相对深度图转换为彩色图像 (BGR) 用于显示/保存"""
    if min_depth is None:
        min_depth = np.min(depth_map)
    if max_depth is None:
        max_depth = np.max(depth_map)
    depth_norm = (depth_map - min_depth) / (max_depth - min_depth + 1e-8)
    depth_norm = np.clip(depth_norm, 0, 1)
    depth_uint8 = (depth_norm * 255).astype(np.uint8)
    try:
        colored = cv2.applyColorMap(depth_uint8, cv2.COLORMAP_INFERNO)
    except:
        colored = cv2.applyColorMap(depth_uint8, cv2.COLORMAP_JET)
    return colored


def get_roi_depth_median(depth_map, xyxy):
    """从全图深度图中提取目标框内（扩大10%框）的中值深度（逆深度）"""
    x1, y1, x2, y2 = map(int, xyxy)
    h, w = depth_map.shape
    pad_w = max(1, int(0.1 * (x2 - x1)))
    pad_h = max(1, int(0.1 * (y2 - y1)))
    x1 = max(0, x1 - pad_w)
    y1 = max(0, y1 - pad_h)
    x2 = min(w, x2 + pad_w)
    y2 = min(h, y2 + pad_h)
    roi = depth_map[y1:y2, x1:x2]
    if roi.size == 0:
        return -1.0
    return float(np.median(roi))


def get_center_avg_depth(depth_map):
    """返回图像中心区域（1/4~3/4 区域）的平均逆深度"""
    h, w = depth_map.shape
    center_roi = depth_map[h//4:3*h//4, w//4:3*w//4]
    if center_roi.size == 0:
        return -1.0
    return float(np.mean(center_roi))


def get_background_avg_depth(depth_map, xyxy):
    """
    返回背景区域（全图除去目标框扩大10%区域）的平均逆深度
    """
    x1, y1, x2, y2 = map(int, xyxy)
    h, w = depth_map.shape
    pad_w = max(1, int(0.1 * (x2 - x1)))
    pad_h = max(1, int(0.1 * (y2 - y1)))
    x1 = max(0, x1 - pad_w)
    y1 = max(0, y1 - pad_h)
    x2 = min(w, x2 + pad_w)
    y2 = min(h, y2 + pad_h)
    mask = np.ones(depth_map.shape, dtype=bool)
    mask[y1:y2, x1:x2] = False
    bg_pixels = depth_map[mask]
    if bg_pixels.size == 0:
        return -1.0
    return float(np.mean(bg_pixels))


# ================= Zigbee 通信函数 =================
def zigbee_receiver_loop():
    """Zigbee 接收线程：持续读取串口并解析深度消息"""
    global zigbee_running, zigbee_ser, remote_target_depth, remote_bg_depth, remote_data_time
    while zigbee_running:
        if zigbee_ser and zigbee_ser.is_open:
            try:
                if zigbee_ser.in_waiting > 0:
                    line = zigbee_ser.readline()
                    try:
                        text = line.decode('utf-8').rstrip('\r\n')
                    except UnicodeDecodeError:
                        text = repr(line)
                    # 解析消息格式：DEPTH:ID,目标深度,背景深度
                    if text.startswith("DEPTH:"):
                        parts = text[6:].split(',')
                        if len(parts) == 3:
                            sender_id = parts[0]
                            try:
                                target_d = float(parts[1])
                                bg_d = float(parts[2])
                                # 存储对方的目标深度和背景深度
                                with remote_lock:
                                    remote_target_depth = target_d
                                    remote_bg_depth = bg_d
                                    remote_data_time = time.time()
                                print(f"[Zigbee] 收到 {sender_id} 目标深度={target_d:.2f} 背景深度={bg_d:.2f}")
                            except ValueError:
                                pass
                    else:
                        print(f"[Zigbee 收到] {text}")
            except Exception as e:
                print(f"[Zigbee 错误] {e}")
        time.sleep(0.05)


def send_zigbee_depth(target_depth, bg_depth):
    """通过 Zigbee 发送深度数据（带限流）"""
    global zigbee_last_send_time, zigbee_ser
    if not zigbee_ser or not zigbee_ser.is_open:
        return
    now = time.time()
    if now - zigbee_last_send_time < ZIGBEE_SEND_INTERVAL:
        return
    msg = f"DEPTH:{MY_ID},{target_depth:.3f},{bg_depth:.3f}\n"
    with zigbee_lock:
        try:
            zigbee_ser.write(msg.encode('utf-8'))
            zigbee_last_send_time = now
        except serial.SerialException:
            pass


# ================= 原有常量 =================
W_img, H_img = 1280, 720
FOV_x, FOV_y = 77, 44
W_real = 0.1
H_real = 1.7
safe_distance = 0.8
k = 1.0
Kp_dx, Kp_dy, Kp_dalt, Kp_dyaw = 0.6, 0.6, 0.7, 0.3
YAW_SEARCH_SPEED = 0.2

dl = None
stop_track_event = threading.Event()


def status_loop(dl_obj, log_path):
    """仅保留低电压警报，不打印常规状态"""
    with open(log_path, 'a', buffering=1) as f:
        while True:
            batt_v = getattr(dl_obj, 'batt_voltage', 0.0)
            if batt_v < 6.7 and batt_v > 0:
                warning = "\033[91m\033[1m!!! BATTERY LOW !!! PLEASE LAND !!!\033[0m"
                print(warning)
                f.write(warning + "\n")
            time.sleep(1)


def tracking_loop(opt, dl_obj, base_dir):
    stop_track_event.clear()
    source, weights, save_txt, imgsz = opt.source, opt.weights, opt.save_txt, opt.img_size
    save_img = not opt.nosave and not source.endswith('.txt')
    webcam = source.isnumeric() or source.endswith('.txt') or source.lower().startswith(
        ('rtsp://', 'rtmp://', 'http://', 'https://'))

    view_img = check_imshow()
    save_dir = base_dir
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)

    # 深度日志文件（保留完整日志，但终端精简）
    depth_log_path = save_dir / f'depth_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    depth_log_f = open(depth_log_path, 'w', encoding='utf-8')
    depth_log_f.write(f"Depth Log started at {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    depth_log_f.write("=" * 80 + "\n")
    depth_log_f.write("NOTE: MiDaS outputs inverse disparity (raw, unitless). Larger value = closer object.\n")
    depth_log_f.write("Fields: timestamp, event, conf, box(x1,y1,x2,y2), center_offset(dx,dy), "
                     "model_dist(m), dx_m, dy_m, d_alt(m), d_yaw(rad), target_depth_inv, background_depth_inv, inference_time(s), depth_time(s)\n")
    depth_log_f.write("=" * 80 + "\n")
    depth_log_f.flush()

    # 设备与模型（YOLO）
    set_logging()
    device = select_device(opt.device)
    half = device.type != 'cpu'
    model = attempt_load(weights, map_location=device)
    stride = int(model.stride.max())
    imgsz = check_img_size(imgsz, s=stride)
    if half:
        model.half()

    # 数据加载
    vid_path, vid_writer = None, None
    depth_vid_path, depth_vid_writer = None, None
    if webcam:
        cudnn.benchmark = True
        dataset = LoadPicamera2(source, img_size=imgsz, stride=stride)
    else:
        dataset = LoadImages(source, img_size=imgsz, stride=stride)

    names = model.module.names if hasattr(model, 'module') else model.names
    colors = [[random.randint(0, 255) for _ in range(3)] for _ in names]

    if device.type != 'cpu':
        model(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model.parameters())))

    print("视觉跟踪 + 深度估计双窗口已启动。")
    if view_img:
        print("按 OpenCV 窗口 'q' 键停止跟踪。")
    print("在本终端输入 'q' + 回车 也能停止跟踪。")
    t0 = time.time()

    lost_start_time = None
    last_no_target_print = 0

    load_midas_once()

    try:
        for path, img, im0s, vid_cap in dataset:
            # 非阻塞检测终端输入 'q'
            while sys.stdin in select.select([sys.stdin], [], [], 0)[0]:
                line = sys.stdin.readline().strip().lower()
                if line == 'q':
                    print("终端输入 'q'，停止跟踪。")
                    stop_track_event.set()
                    break
            if stop_track_event.is_set():
                break

            img = torch.from_numpy(img).to(device)
            img = img.half() if half else img.float()
            img /= 255.0
            if img.ndimension() == 3:
                img = img.unsqueeze(0)

            # YOLO 推理
            t1 = time_synchronized()
            pred = model(img, augment=opt.augment)[0]
            pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres,
                                       classes=[0], agnostic=opt.agnostic_nms)
            t2 = time_synchronized()
            inference_time = t2 - t1

            det = pred[0] if len(pred) > 0 else []
            im0 = im0s.copy() if isinstance(im0s, np.ndarray) else im0s[0].copy()

            # 全图深度估计
            rgb_for_depth = cv2.cvtColor(im0, cv2.COLOR_BGR2RGB)
            t_depth_start = time.time()
            depth_map = run_midas_full(rgb_for_depth)
            depth_time = time.time() - t_depth_start

            colored_depth = depth_to_colormap(depth_map)

            # 控制决策
            if len(det) == 0:
                now = time.time()
                if lost_start_time is None:
                    lost_start_time = now
                    dl_obj.set_pose(0, 0, 0, 0)
                    print("[丢失目标] 悬停")
                else:
                    elapsed = now - lost_start_time
                    if elapsed >= 5.0:
                        dl_obj.set_pose(0, 0, 0, YAW_SEARCH_SPEED)
                        if elapsed - 5.0 < 0.5:
                            print("[自动旋转] 搜寻目标")
                    else:
                        dl_obj.set_pose(0, 0, 0, 0)

                if time.time() - last_no_target_print >= 1.0:
                    center_avg = get_center_avg_depth(depth_map)
                    print(f"[MiDaS] 中心深度: {center_avg:.2f}")
                    last_no_target_print = time.time()
            else:
                if lost_start_time is not None:
                    print("[恢复跟踪]")
                    lost_start_time = None
                    dl_obj.set_pose(0, 0, 0, 0)

                det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0.shape).round()
                max_conf_idx = torch.argmax(det[:, 4])

                for idx, (*xyxy, conf, cls) in enumerate(det):
                    if save_txt:
                        gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()
                        line = (cls, *xywh, conf) if opt.save_conf else (cls, *xywh)
                        frame_id = getattr(dataset, 'frame', 0)
                        txt_path = save_dir / 'labels' / f"{Path(path).stem}_{frame_id}" if dataset.mode != 'image' else save_dir / 'labels' / Path(path).stem
                        txt_path = str(txt_path) + '.txt'
                        with open(txt_path, 'a') as f:
                            f.write(('%g ' * len(line)).rstrip() % line + '\n')

                    if idx == max_conf_idx:
                        label = f'{names[int(cls)]} {conf:.2f}'
                        plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=3)

                        # 控制量计算（基于二维码测距）
                        f_x = f_y = 2120
                        x1, y1, x2, y2 = xyxy
                        W_qr = x2 - x1
                        H_qr = y2 - y1
                        cx_qr = (x1 + x2) / 2
                        cy_qr = (y1 + y2) / 2
                        cx_img = W_img / 2
                        cy_img = H_img / 2
                        dx = cx_qr - cx_img
                        dy = cy_qr - cy_img
                        angle_x_rad = k * np.arctan(dx / f_x)
                        dz_m = k * (W_real * f_x) / W_qr      # 距离（米）
                        dy_m = k * (dy / f_y) * dz_m
                        dx_m = k * (dx / f_x) * dz_m
                        dx_1 = dz_m - safe_distance
                        dy_1 = dx_m
                        d_alt_1 = -dy_m
                        d_yaw = angle_x_rad

                        # 深度值（逆深度）
                        target_depth_inv = get_roi_depth_median(depth_map, xyxy)
                        background_depth_inv = get_background_avg_depth(depth_map, xyxy)

                        # 发送深度数据
                        send_zigbee_depth(target_depth_inv, background_depth_inv)

                        # 协同决策：归一化背景深度比较
                        rel_bg_self = None
                        rel_bg_remote = None
                        decision = "正常跟踪"
                        # 避免除零
                        if target_depth_inv > 1e-6:
                            rel_bg_self = background_depth_inv / target_depth_inv
                        # 获取对方数据（未超时）
                        with remote_lock:
                            r_target = remote_target_depth
                            r_bg = remote_bg_depth
                            r_time = remote_data_time
                        if r_target is not None and r_bg is not None and r_target > 1e-6 and (time.time() - r_time) < 2.0:
                            rel_bg_remote = r_bg / r_target
                            if rel_bg_self is not None and rel_bg_remote is not None:
                                if rel_bg_self > rel_bg_remote:
                                    # 自己的相对背景深度更大 → 自己离背景更近（逆深度大）→ 后退
                                    decision = "后退（前方背景更近）"
                                    dx_1 = -0.3   # 后退速度 0.3 m/s
                                    dy_1 = 0.0
                                    d_alt_1 = 0.0
                                    d_yaw = 0.0
                                else:
                                    decision = "前进/正常跟踪"
                        # 输出精简信息
                        print(f"[目标] 距离={dz_m:.2f}m | 目标深度={target_depth_inv:.2f} 背景深度={background_depth_inv:.2f} | "
                              f"相对背景: self={rel_bg_self:.3f}" + (f" remote={rel_bg_remote:.3f}" if rel_bg_remote is not None else "") +
                              f" | {decision}")

                        # 发送控制指令
                        try:
                            dl_obj.set_pose(Kp_dx * dx_1, Kp_dy * dy_1,
                                            Kp_dalt * d_alt_1, Kp_dyaw * d_yaw)
                        except Exception as e:
                            print(f"控制发送失败: {e}")

                        # 详细日志写入文件
                        depth_log_f.write(
                            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] DETECT "
                            f"conf={conf:.3f} box=({x1},{y1},{x2},{y2}) "
                            f"center_offset=({dx:.1f},{dy:.1f}) "
                            f"model_dist={dz_m:.3f}m "
                            f"dx_m={dx_m:.3f}m dy_m={dy_m:.3f}m d_alt={d_alt_1:.3f}m "
                            f"d_yaw={d_yaw:.3f}rad "
                            f"target_depth_inv={target_depth_inv:.2f} "
                            f"background_depth_inv={background_depth_inv:.2f} "
                            f"inference_time={inference_time:.3f}s depth_time={depth_time:.3f}s\n"
                        )
                        depth_log_f.flush()

            # 双窗口显示
            if view_img:
                cv2.namedWindow("YOLOv5 Tracking", cv2.WINDOW_NORMAL)
                cv2.resizeWindow("YOLOv5 Tracking", 640, 360)
                cv2.imshow("YOLOv5 Tracking", im0)

                cv2.namedWindow("MiDaS Depth", cv2.WINDOW_NORMAL)
                cv2.resizeWindow("MiDaS Depth", 640, 360)
                cv2.imshow("MiDaS Depth", colored_depth)

                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'):
                    print("窗口 'q' 键按下，停止跟踪。")
                    stop_track_event.set()
                    break
            else:
                cv2.waitKey(1)

            # 保存视频（完整保留原始逻辑）
            if save_img:
                if dataset.mode == 'image':
                    cv2.imwrite(str(save_dir / Path(path).name), im0)
                else:
                    if vid_path != str(save_dir / 'output.mp4'):
                        vid_path = str(save_dir / 'output.mp4')
                        if isinstance(vid_writer, cv2.VideoWriter):
                            vid_writer.release()
                        if vid_cap:
                            fps = vid_cap.get(cv2.CAP_PROP_FPS)
                            w = int(vid_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
                            h = int(vid_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
                        else:
                            fps, w, h = 15, im0.shape[1], im0.shape[0]
                        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                        vid_writer = cv2.VideoWriter(vid_path, fourcc, fps, (w, h))
                    vid_writer.write(im0)

                if depth_vid_path != str(save_dir / 'depth_output.mp4'):
                    depth_vid_path = str(save_dir / 'depth_output.mp4')
                    if isinstance(depth_vid_writer, cv2.VideoWriter):
                        depth_vid_writer.release()
                    dh, dw = colored_depth.shape[:2]
                    depth_fps = fps if vid_cap else 15
                    depth_vid_writer = cv2.VideoWriter(depth_vid_path, fourcc, depth_fps, (dw, dh))
                depth_vid_writer.write(colored_depth)

            if stop_track_event.is_set():
                break

    finally:
        depth_log_f.close()
        if vid_writer is not None:
            vid_writer.release()
        if depth_vid_writer is not None:
            depth_vid_writer.release()
        cv2.destroyAllWindows()
        if save_txt or save_img:
            s = f"\n{len(list(save_dir.glob('labels/*.txt')))} labels saved to {save_dir / 'labels'}" if save_txt else ''
            print(f"结果保存至 {save_dir}{s}")
        print(f"跟踪循环结束 (总耗时 {time.time() - t0:.3f}s)")


def execute_command(cmd_char, opt, base_dir):
    global dl, stop_track_event
    cmd_char = cmd_char.lower()
    if cmd_char == 'a':
        dl.set_arm()
        print("解锁(arm)指令已发送")
    elif cmd_char == 'd':
        dl.set_disarm()
        print("上锁(disarm)指令已发送")
    elif cmd_char == 't':
        dl.set_takeoff()
        print("起飞(takeoff)指令已发送")
    elif cmd_char == 'l':
        dl.set_land()
        print("降落(land)指令已发送")
    elif cmd_char == 'k':
        stop_track_event.set()
        time.sleep(0.5)
        stop_track_event.clear()
        tracking_loop(opt, dl, base_dir)
    elif cmd_char == 'h':
        dl.set_pose(0, 0, 0, 0)
        stop_track_event.set()
        print("悬停(halt)，跟踪已停止")
    else:
        print("? 支持命令: a(解锁) d(上锁) t(起飞) l(降落) k(跟踪+深度) h(悬停)")


def main():
    global dl, zigbee_ser, zigbee_running

    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str,
                        default='/home/pi/YOLOv5-Lite/weights/best.pt')
    parser.add_argument('--source', type=str, default='0')
    parser.add_argument('--img-size', type=int, default=416)
    parser.add_argument('--conf-thres', type=float, default=0.06)
    parser.add_argument('--iou-thres', type=float, default=0.2)
    parser.add_argument('--device', default='')
    parser.add_argument('--view-img', action='store_true')
    parser.add_argument('--save-txt', action='store_true')
    parser.add_argument('--save-conf', action='store_true')
    parser.add_argument('--nosave', action='store_true')
    parser.add_argument('--classes', nargs='+', type=int)
    parser.add_argument('--agnostic-nms', action='store_true')
    parser.add_argument('--augment', action='store_true')
    parser.add_argument('--update', action='store_true')
    parser.add_argument('--project', default='runs/detect')
    parser.add_argument('--name', default='exp')
    parser.add_argument('--exist-ok', action='store_true')
    opt = parser.parse_args()
    print("参数:", opt)
    check_requirements(exclude=('pycocotools', 'thop'))

    base_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))
    base_dir.mkdir(parents=True, exist_ok=True)

    status_log_path = base_dir / f"status_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"

    # 初始化 Zigbee
    try:
        zigbee_ser = serial.Serial(ZIGBEE_PORT, ZIGBEE_BAUD, timeout=0.5)
        print(f"[Zigbee] 已打开 {ZIGBEE_PORT} @ {ZIGBEE_BAUD} baud，本机ID: {MY_ID}")
        zigbee_thread = threading.Thread(target=zigbee_receiver_loop, daemon=True)
        zigbee_thread.start()
    except Exception as e:
        print(f"[警告] Zigbee 初始化失败: {e}，无线通信禁用")
        zigbee_ser = None

    dl = datalink()
    threading.Thread(target=dl.drone, daemon=True).start()
    threading.Thread(target=dl.heartbeat, daemon=True).start()
    threading.Thread(target=status_loop, args=(dl, str(status_log_path)), daemon=True).start()

    print("\n===== 无人机单键遥控 + 深度跟踪双窗口 + 集群协同（归一化背景深度）=====")
    print("命令: a(解锁) d(上锁) t(起飞) l(降落) k(跟踪+深度) h(悬停)")
    print(f"输出目录: {base_dir}\n")

    try:
        while True:
            cmd = input(">> ").strip().lower()
            if cmd:
                if len(cmd) == 1:
                    execute_command(cmd, opt, base_dir)
                else:
                    print("只接受单字母命令: a/d/t/l/k/h")
    except KeyboardInterrupt:
        print("\n用户退出。")
    finally:
        stop_track_event.set()
        zigbee_running = False
        if zigbee_ser and zigbee_ser.is_open:
            zigbee_ser.close()
        print("程序已结束。")


if __name__ == '__main__':
    main()